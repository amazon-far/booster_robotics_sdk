# Publishing `far-booster-robotics-sdk` to PyPI
...
- **Distribution name:** `far-booster-robotics-sdk` (the `far-` prefix avoids
  colliding with any upstream Booster project).
- **Import name (unchanged):** `from booster_robotics_sdk_python import ...`.
- **Wheels:** `manylinux_2_34` for Python 3.8 / 3.10 / 3.11 / 3.12 on x86_64 +
  aarch64, with the FastDDS runtime libraries (and `libtinyxml2.so.9`) bundled
  in by `auditwheel`.
...
3. Fill in **exactly**:
   - **PyPI project name:** `far-booster-robotics-sdk`
   - **Owner:** `amazon-far`
...
pip install --index-url https://test.pypi.org/simple/ far-booster-robotics-sdk
