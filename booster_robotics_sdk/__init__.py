...
# `import booster_robotics_sdk` succeed but expose an empty module, which is a
...
try:
    from .booster_robotics_sdk_python import *  # noqa: F401,F403
    from . import booster_robotics_sdk_python as _ext

    __all__ = getattr(_ext, "__all__", [name for name in dir(_ext) if not name.startswith("_")])
except ImportError as exc:
    raise ImportError(
        "Failed to load the compiled 'booster_robotics_sdk_python' extension. "
        "If you installed from a wheel this indicates a broken install (e.g. a "
