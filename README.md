...
The Python bindings are published to PyPI as **`far-booster-robotics-sdk`** (the
`far-` prefix avoids colliding with upstream Booster packages). The import name
is unchanged:

```bash
pip install far-booster-robotics-sdk
```
...
Wheels are prebuilt `manylinux` binaries (Python 3.8/3.10/3.11/3.12, x86_64 and
